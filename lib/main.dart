      ...
      child: ListView(
        children: [
          ...
          TextButton(
            onPressed: () {
              Navigator.pop(context);
              Navigator.push(context, MaterialPageRoute(builder: (context) => RecycleMap()));
            },
            child: ListTile(
        ...
        builder: (context, AsyncSnapshot<DocumentSnapshot> snapshot) {
          ...
          if (snapshot.connectionState == ConnectionState.done) {
            var pins = (snapshot.data!["loc"] as List<dynamic>).map((e){
              return Marker(alpha: 1, position: LatLng(e.latitude, e.longitude), markerId: MarkerId("example"));
            }).toList();
